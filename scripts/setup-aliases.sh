...
echo -e "${BLUE}🔧 Installing Telos Idea Matrix and setting up aliases...${NC}"
...
mkdir -p "$LOCAL_BIN"
echo -e "${GREEN}✅ Created local bin directory: $LOCAL_BIN${NC}"
...
if [ -f "$BINARY_PATH" ]; then
    ...
    if [ ! -f "$LOCAL_BIN/tm" ] || ! cmp -s "$BINARY_PATH" "$LOCAL_BIN/tm"; then
        ...
        echo -e "${GREEN}✅ Binary installed to $LOCAL_BIN/tm${NC}"
    else
        echo -e "${YELLOW}⚠️  Binary already exists and is up to date${NC}"
    fi
    ...
    if "$LOCAL_BIN/tm" --help >/dev/null 2>&1; then
        ...
        echo -e "${RED}❌ Binary doesn't work as expected${NC}"
    fi
else
...
if [[ ":$PATH:" != *":$LOCAL_BIN:"* ]]; then
    ...
    if ! grep -q "$LOCAL_BIN" "$RC_FILE" 2>/dev/null; then
        ...
        echo -e "${GREEN}✅ Added $LOCAL_BIN to PATH in $RC_FILE${NC}"
    fi
...
add_to_shell_config() {
    ...
    echo -e "${BLUE}📝 Adding aliases to $rc_file${NC}"
    
    # Backup the file first
    if [ -f "$rc_file" ]; then
        cp "$rc_file" "${rc_file}.backup-$(date +%Y%m%d-%H%M%S)"
        echo -e "${GREEN}✅ Backup created: ${rc_file}.backup-*${NC}"
    fi
    ...
    if ! grep -q "source.*tm-alias.sh" "$rc_file" 2>/dev/null; then
        ...
        echo -e "${YELLOW}⚠️  Aliases already present in $rc_file${NC}"
    fi
}

# Main function
main() {
    echo -e "${BLUE}📋 Step 1: Creating alias function script${NC}"
    create_alias_function
    
    # Make the alias script executable
    chmod +x "$PROJECT_DIR/tm-alias.sh"
    echo -e "${GREEN}✅ Alias script created and made executable${NC}"
    
    echo -e "${BLUE}📋 Step 2: Adding aliases to shell configuration${NC}"
    ...
    echo -e "${BLUE}📋 Step 3: Creating installation message${NC}"
    echo "";
    echo -e "${GREEN}🎉 Telos Idea Matrix installed successfully!${NC}"
    echo "";
    ...
    echo "";
    echo -e "${BLUE}📝 The binary and aliases will be available in new terminal sessions${NC}"
}
